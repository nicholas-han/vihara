...
typedef std::shared_ptr<PathGenerator> SPtrPathGenerator;
...
inline void EulerPathGenerator<NRNG>::next(Matrix& pricePath)
{
  ...
  for (size_t j = 0; j < nfactors_; ++j) {
    ...
    // row iterator
    //        Matrix::iterator1 rit = cit.begin();
    //        nrng_.next(cit.begin(), cit.begin() + 5);
    //        ++cit;
  }
  // now multiply them with sqrt(DeltaT)
  for (size_t j = 0; j < nfactors_; ++j) {
    for (size_t i = 0; i < ntimesteps_; ++i)
      pricePath(i, j) *= sqrtDeltaT_(i);
  }
