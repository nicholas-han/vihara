...
`FinancialAccount` 表示现实 Financial / custody account。

MVP 不设计 parent/master/group hierarchy。未来若需要 broker-level aggregation、sub-account grouping 或更复杂 custody hierarchy，作为独立专项重新设计，而不是预设 parent-child tree。

FinancialAccount 在不同 domain 中承担不同角色：

```text
Transaction
→ participating FinancialAccount
...
因此不得把 `FinancialAccount` 本身改名为 `Location`。
...
```text
owner_id = Forbidden
financial_account_id = Mandatory
```
...
```text
(position_id, financial_account_id)
```
...
```text
(position_id, owner_id, financial_account_id)
```
...
不同 account / owner bucket 的 lots 不混用。
...
```text
Create lot in:
(position_id, SELF, financial_account_id)
```
