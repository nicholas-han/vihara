...
先选 Financial Account，再加载该账户 scopes。零 scope 时阻止 Trade 提交并指引完成参考数据设置；一个 scope 自动选中，DEFAULT 控件隐藏；多个 scopes 必须显式选择 exactly one。切换账户清除旧 scope 与 preview。

Trade preview / detail 展示选定 scope（DEFAULT 标签隐藏）及只读 tax classification。表单不提供独立 TaxScheme 选择器；API 提交稳定 position_scope_id，后端重新检查账户归属。
...
MVP 支持 List / Create。字段为 account_code、display_name、country_or_region（可空）、institution_type（BANK / BROKER-DEALER / INSURER）。账户非主键字段允许数据库 correction，但 UI 不提供普通编辑、删除或 archive 入口。

支持 Position 的账户创建时至少有一个 scope：简单账户使用 DEFAULT；复杂账户由受控初始化工具配置 scopes。纯现金 BANK 可以没有 scope。Settings 只读显示 scopes、TaxScheme 和当前已登记的 external references；MVP 不提供 TaxScheme 管理或 scope tax treatment 修改 workflow。

外部号码只用于 import/provenance，不显示成 Holdings 子账户；不在 Trade 表单按 tax label 临时拼装 location。
...
Trade 还必须解析到唯一 PositionScope：使用 account_code / external account number / source tax label / 用户显式映射共同确定。无法唯一解析时留在 staging，不 canonicalize；external reference 不与 scope 建立一对一关系。单 scope 可自动解析，多 scope 缺少选择必须报错。
