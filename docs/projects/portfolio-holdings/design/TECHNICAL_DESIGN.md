...
- Investment canonical key：(position_id, SELF, position_scope_id)；账户展示由 PositionScope → FinancialAccount 汇总。数量来自 LOCATION，SELF ownership 作对账；账户成本来自 active lot/allocation。
...
| Accounts | GET/POST `/api/accounts`；不提供普通 PATCH 入口 |
