...
**Consequences.** `vol-arb-v1` touches three folders; decisions for all three are logged here in `docs/modules/portfolio_manager` as the hub until `forecaster` / `strategies` grow their own docs.

## ADR-9 — Canonical text CSVs in vihara-data; SQLite becomes a derived index
**Decision.** The canonical trade/dividend/cashflow/snapshot/checkpoint/fx records are append-only CSVs in the private `vihara-data` repo under `portfolio/` (see `docs/modules/portfolio_manager/portfolio-data-layout.md`); the records SQLite database is a disposable index rebuilt from them by `python -m portfolio_manager.records rebuild`. `PORTFOLIO_DB_PATH` defaults to `$VIHARA_DATA_DIR/build/portfolio.sqlite3`. Confirmed by the founder 2026-07-14 as the repo-wide storage direction (text canonical + SQLite derived), together with the `ledger` module.
...
## ADR-12 — Instrument identity is opaque and independent from ticker aliases
**Decision.** Portfolio `instrument_id` uses `ins_` plus 22 lowercase Crockford Base32 characters. It never embeds symbol, market, venue, or issuer name. Canonical instrument-bearing CSVs require this id; adapters resolve mutable ticker aliases against instrument_manager as of the record date and must reject missing or ambiguous matches. The ledger encodes the same opaque payload as `I<UPPERCASE_PAYLOAD>`.

**Rationale.** Tickers change and can later be reassigned to a different security. Treating `AAPL.US` as identity would either rewrite history or silently attach old transactions to the wrong instrument.

**Consequences.** `symbol` and `market` remain audit/display fields, not keys. Existing symbol-derived portfolio data requires a one-time migration through the instrument resolver. Fee input is simultaneously simplified to the single required `transaction_fees` field; detailed fee taxonomy is deferred.
