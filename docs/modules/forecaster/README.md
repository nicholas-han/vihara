...
> Documentation location: repository `docs/`. Source code remains in `forecaster`. Shell commands retain their stated working directory.
...
[Portfolio decisions](../portfolio_manager/decisions.md)
(ADR-5, ADR-6). Independent Forecaster designs belong alongside this file in root `docs/modules/forecaster/`.
...
numpy-only for the built sub-packages. From the `forecaster/` source directory, `pip install -e .[econometrics|ml|dl|rl]`
